...
def _apply_bcm( Y0: np.ndarray , X0: np.ndarray , X1f: np.ndarray , X1p: np.ndarray , bc_method: AbstractBC, bc_method_kwargs: dict[str,Any] = {}, n_multi_dims: int = 0 ) -> np.ndarray:
...
#    ## For dev
#    print( f"{Y0.shape} / {X0.shape} / {X1f.shape} / {X1p.shape} / {n_multi_dims}" )
    
    ## Find uni and multi-axis
    tdim  = Y0.ndim - 1 - n_multi_dims
    
    ## Loop
    for pidx in itt.product(*[range(s) for s in Y0.shape[:tdim]]):
        idx = pidx + (slice(None),) + tuple([slice(None) for _ in range(n_multi_dims)])
        shp = [1 for _ in range(len(pidx))] + [-1] + [Y0.shape[s+tdim+1] for s in range(n_multi_dims)]
...
def apply_bcm( Y: xr.DataArray, X: xr.DataArray,
              ...
              period_calibration: tuple[str | int,str | int],
              range_projection:tuple[str | int,str | int] | None = None,
              window_projection: tuple[int,int,int] = (5,10,5),
              time_dim: str = "time",
              seas_cycle: str = "month",
              seas_cycle_window: int = 15,
              multi_dims: Sequence | str = tuple(),
              chunks: dict[str,int | str] | None = None,
              bc_method_kwargs: dict[str,Any] = {} ) -> xr.DataArray:
    ...
    cal0 = int(period_calibration[0])
    cal1 = int(period_calibration[1])
    ...
    if range_projection is None:
        range_projection = (bleft,bright)
    prj0 = int(range_projection[0])
    prj1 = int(range_projection[1])
    ...
    try:
        if not len(window_projection) == 3:
            raise ValueError
        for w in window_projection:
            if not isinstance( w , int ):
                raise ValueError
        wl,wm,wr = window_projection
    except Exception as e:
        raise ValueError(f"Incoherent window_projection '{window_projection}' {e}")
    ...
    match seas_cycle:
        ...
        case "month":
            ...
            grp_name = "month"
        case "window":
            groups = [ [ ( (d + w) % 366 ) + 1 for w in range(-seas_cycle_window,seas_cycle_window+1,1) ] for d in range(366)]
            grp_name = "dayofyear"
        case _:
    ...
    if isinstance( multi_dims , str ):
        multi_dims = (multi_dims,)
    multi_dims = tuple(multi_dims)
    for d in multi_dims:
        if d not in X.dims:
            raise ValueError(f"Dimension '{d}' of multi_dims is not in X")
    if time_dim in multi_dims:
        raise ValueError("Time dimension '{time_dim}' can not be a dimension of X or Y")
    ...
    ## Dask arguments
    input_core_dims  = [(tdim,) + multi_dims for tdim in [f"{time_dim}0",f"{time_dim}0",f"{time_dim}1f",f"{time_dim}1p"] ]
    output_core_dims = [(f"{time_dim}1p",) + multi_dims]
    ...
    for tf0,tp0,tp1,tf1 in yearly_window( prj0 , prj1 , wl , wm , wr , bleft , bright ):
        ...
        ## Loop on groups
        for grps in groups:
#            logger.info(f"{tf0} / {tp0} / {tp1} / {tf1} | Group {grps}")
            ...
            Z1ps = xr.apply_ufunc( _apply_bcm, Y0s.chunk(chunks) , X0s.chunk(chunks) , X1fs.chunk(chunks) , X1ps.chunk(chunks) ,
                                  ...
                                  dask = "parallelized",
                                  kwargs = { "bc_method": bc_method , "bc_method_kwargs": bc_method_kwargs , "n_multi_dims": len(multi_dims) },
