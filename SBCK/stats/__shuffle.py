...
class SchaakeShuffle:##{{{
    """Match the rank structure of X with them of Y by reordering X. Work in
    multivariate case, but rank  of each features are reordered independantly.
    """
    
    _Y0: _Array | None

    def __init__( self , Y0: _Array | None = None ) -> None:##{{{
        """
        Parameters
        ----------
    ...
    def fit( self , Y0: _Array ) -> None:##{{{
        """Fit the reference ranks structure
    ...
    def _predict( self , Y0: _Array , X0: _Array ) -> _Array:##{{{
        X0 = X0.squeeze()
        Y0 = Y0.squeeze()
...
def schaake_shuffle( Y0: _Array , X0: _Array ) -> _Array:##{{{
    """Match the rank structure of X0 with them of Y0 by reordering X0. Work in
    multivariate case, but ranks of each features are reordered independantly.
    
    Note: This function just call the class SBCK.stats.SchaakeShuffle
...
class SchaakeShuffleRef(SchaakeShuffle):##{{{
    """Match the rank structure of X with them of Y by reordering X, but fix
    one features to keep the structure of X.
    """
    
    _ref: int
    
    def __init__( self , ref: int , Y0: _Array | None = None ) -> None:##{{{
        """
        Parameters
        ----------
        ref : int
...
class MVQuantilesShuffle: ##{{{
    ...
    col_ucond: Sequence[int]
    lag_search: int
    lag_keep: int
    qY: _Array
    bsYc: _Array

    def __init__( self , col_cond: Sequence[int] = [1] , lag_search: int = 1 , lag_keep: int = 1 ) -> None: ##{{{
        """
        Parameters
        ----------
        col_cond : Sequence[int]
