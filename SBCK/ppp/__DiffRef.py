...
class PreserveOrder(PrePostProcessing):##{{{
	"""
	SBCK.ppp.PreserveOrder
	======================
	
	The inverse transform of this PPP sort the data of the column 'cols' along
	rows. It is useful for example when tas, tasmin and tasmax are corrected to
	ensure their order.
...
class DeltaVars(PrePostProcessing):##{{{
	...
	Similar to SBCK.ppp.DeltaRef, but diff columns are replaced by diff column
