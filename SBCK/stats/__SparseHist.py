...
def bin_width_estimator( *args: np.ndarray , method: str = "auto" ) -> np.ndarray:##{{{
	"""
	SBCK.stats.bin_width_estimator
	==============================
...
class BaseHist:##{{{
	...
	"""
	def __init__( self , c: np.ndarray , p: np.ndarray ) -> None:##{{{
		self._p = p
		self._c = c
...
class SparseHist:##{{{
	...
	def __init__( self , X: np.ndarray , bin_width: np.ndarray | list | float | None = None , bin_origin: np.ndarray | list | float = 0 ) -> None:##{{{
		"""
		SBCK.stats.SparseHist.__init__
		==============================
		Sparse Histogram class, interface to a c++ class.
	...
	def argwhere( self , X: np.ndarray ) -> np.ndarray:##{{{
		"""
		SBCK.stats.SparseHist.argwhere
		==============================
