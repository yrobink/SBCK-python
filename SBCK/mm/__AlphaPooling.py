...
class Univariate_AlphaPooling(UnivariateBC):##{{{
	...
	def _normalization( self , Y0: np.ndarray , X0s: Sequence[np.ndarray] , X1s: Sequence[np.ndarray] ) -> tuple[Sequence[np.ndarray],Sequence[np.ndarray]]:##{{{
	...
	def _fit_pooling_optim( self , p , rvY0 , rvXs ):##{{{
	...
	def _fit_pooling( self , rvY0 , rvXs ):##{{{
	...
	def fit( self , Y0: np.ndarray , *args: Sequence[np.ndarray] ) -> Self:##{{{
	...
	def predict( self , *args: Sequence[np.ndarray] ) -> Sequence[np.ndarray]:##{{{
...
class AlphaPooling(MultiUBC):##{{{
	...
	def fit( self , Y0: np.ndarray , X0s : Sequence[np.ndarray] , X1s: Sequence[np.ndarray] ) -> Self:##{{{
		args = [Y0] + X0s + X1s
		return super().fit( *args )
	##}}}
	
	def predict( self , X1s : Sequence[np.ndarray] , X0s: Sequence[np.ndarray] | None ) -> Sequence[np.ndarray] | tuple[Sequence[np.ndarray],Sequence[np.ndarray]]:##{{{
		args = X1s
		if X0s is not None:
			args = args + X0s
