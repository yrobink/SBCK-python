...
def _to_SparseHist( func: Callable ) -> Callable:##{{{
    """Decorators for distances functions to transform input in SparseHist
    """
    def wrapper( muX: SparseHist | _Array , muY: SparseHist | _Array , *args , **kwargs ) -> float:
...
def chebyshev( muX: SparseHist , muY: SparseHist , normalized: bool = True ) -> float:##{{{
    """Chebyshev distance between SparseHist, defined by
...
def wasserstein( muX: SparseHist , muY: SparseHist , p: float = 2. , ot: POTemd = POTemd() , metric: str = "euclidean" ) -> float:##{{{
    """Compute the Wasserstein metric between two sparse histograms. If ot is a
    Sinkhorn algorithm, the dissimilarity is returned.
