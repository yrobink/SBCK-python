...
class EnsITE(AbstractBC):##{{{
    ...
    Main class for independent ensemble bias correction. It is equivalent to
    correct each members independently.
    ...
    Notes
    -----
    The numbers of run of X0 and X1 must be equal.
    
    """
    
    def __init__( self , bcm: AbstractBC , bcm_kwargs: dict[Any] = {} ) -> None:##{{{
        """
        Parameters
        ----------
        bcm: SBCK.AbstractBC
    ...
    def _reshapeX( self , X: _Array ) -> _Array :##{{{
        """Reshape such that the shape is (run,time,ndim)"""
    ...
    def predict( self , X1: _Array , X0: _Array | None = None ) -> _Array | tuple[_Array,_Array]:##{{{
        ...
        X0: numpy.ndarray | None
            Biased model in calibration period (optional)
        
        Returns
        -------
        Z1: numpy.ndarray
            Correction of the biased model in projection period
...
class EnsCTE(AbstractBC):##{{{
    ...
    Main class for ensemble bias correction as defined by Vaittinada A. et al
    (2021) (doi:10.1038/s41598-021-82715-1).
