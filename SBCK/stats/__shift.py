...
class Shift:##{{{
    """Shift class used to to transform a
    dataset X to [X[0:size-lag],X[1:size-lag+1],...]
    """
    ...
    def _inverse_by_row( self , Xs: _Array ) -> _Array:##{{{
        """Inverse transform by row
    ...
    def inverse( self , Xs: _Array , method: str | None = None ) -> _Array:##{{{
        """Inverse transform
